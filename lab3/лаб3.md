...
1. Создала таблицу `iso_test (id INT, data TEXT)` и вставила одну строку. В сеансе 1 начала транзакцию с уровнем `READ COMMITTED` и выполнила `SELECT iso_test;. * FROM iso_test;`. В сеансе 2 удалила строку и зафиксировал изменения (`DELETE ...; COMMIT;`). В сеансе 1 выполнила тот же `SELECT` повторно. Вывело 0 строк. Завершила транзакцию в сеансе 1.
...
3. В сеансе 1 начала транзакцию и создала новую таблицу `new_table`, вставила в нее строку. Не фиксировала. В сеансе 2 выполнила `SELECT * FROM new_table;`. Выдало ошибку о несуществовании таблицы. Зафиксировала транзакцию в сеансе 1. Повторила запрос в сеансе 2, таблица появилась. Повторила процесс, но вместо фиксации откатила транзакцию в сеансе 1. После `ROLLBACK;` таблица также нету в сеансе 2.

4. В сеансе 1 начала транзакцию и выполнила `SELECT * FROM iso_test;`. В сеансе 2 попыталась выполнить `DROP TABLE iso_test;`. Удалить таблицу не получится, пока не завершим транзакцию в первом сеансе.
...
2. В сеансе 1 начала транзакцию с уровнем `Repeatable Read` (пока без запросов). В сеансе 2 удалила все строки из `phantom_test` и зафиксировала. В сеансе 1 выполнила `SELECT * FROM phantom_test;`. Удалённые строки вывелись. В сеансе 1 выполнила запрос `SELECT * FROM pg_database;` (не касаясь `phantom_test`). При следующем запросе также вывело удалённые строки.
...
1. Создала таблицу `version_test (id INT)`. Вставила одну строку. Дважды обновила эту строку (`UPDATE ...`), а потом удалила её (`DELETE`). Используя расширение `pageinspect` (`heap_page_items`), сейчас версий строк 3. Все версии погашены, так как t_xmax не равен 0. По t_ctid видно, что первая версия строки ссылается на вторую версия, так как вторая удалила первую версию. Вторая версия переадресовывает на третью версию. Так как это была последняя версия строки она ссылается на саму себя.

2. Определила, в какой странице (блоке) находится строка в `pg_class`, описывающая саму таблицу `pg_class`. Используя `pageinspect`, актуальных версий строк 2, т.к. только у двух строк t_xmax = 0.
...
В ходе выполнения работы изучила принципы многоверсионного управления конкурентным доступом (MVCC) в PostgreSQL. Были получены практические навыки наблюдения за работой MVCC, анализа версий строк, снимков данных и уровней изоляции транзакций. Также освоено использование расширений и системных представлений для исследования внутренней структуры данных.
